...
4. **Grafik** — komponen `x-line-chart` (SVG murni server-side, TANPA JS/Alpine, garis ambang putus-putus merah). Revisi client 2026-07-09: di dashboard SELALU tampil (bukan cuma saat difilter) — tren MBW/ADG/SR per tanggal sampling (rata-rata lintas kolam terfilter; saat difilter 1 kolam otomatis jadi nilai kolam itu) + 4 grafik air mingguan: ammonia, TAN, kepadatan vibrio hijau, rasio V/B. Versi per-kolam lengkap tetap di Hub Siklus & Uji Lab.
