...
> ⚠️ **STATUS SEMENTARA (2026-07-04)**: `routes/web.php` sengaja dilonggarkan buat testing — role `operasional` ditambahkan ke akses tulis "Kualitas Air Mingguan" (asli: `lab` saja) dan "Sampling & Pertumbuhan" (asli: `analis` saja), supaya 1 akun bisa coba semua menu. Cari komentar `TEMP TESTING (2026-07-04)` di `routes/web.php` dan `tests/Feature/RoleAccessTest.php` — **WAJIB dibalikin ke role asli sebelum deploy/dipakai beneran**, karena ini melanggar pemisahan role asli PRD Bagian 3.
